...
- [ ] URL 파라미터 (invoiceId)가 페이지에서 추출됨
- [ ] 회사 정보 섹션이 올바르게 표시됨
- [ ] 고객 정보 섹션이 올바르게 표시됨
- [ ] 인보이스 메타 정보 (번호, 날짜, 상태)가 표시됨
- [ ] 라인 아이템 테이블이 올바르게 표시됨
- [ ] 소계, 세금, 합계가 올바르게 계산 및 표시됨
- [ ] PDF 다운로드 버튼이 표시됨 (기능은 플레이스홀더)
- [ ] 목록으로 돌아가기 링크가 동작함
- [ ] A4 인쇄 레이아웃이 최적화됨
- [ ] 로딩 스켈레톤이 구현됨
- [ ] 모든 테스트 통과
...
- [ ] `app/presentation/routes/invoices/_data/dummy-invoice-detail.ts` 생성
  - 단일 Invoice 더미 데이터 (모든 필드 포함)
  - 5-7개의 InvoiceLineItem 더미 데이터
  - CompanyInfo 더미 데이터
- [ ] InvoiceWithLineItems 복합 타입 사용
...
- [ ] `app/presentation/components/invoice/invoice-detail-skeleton.tsx` 생성
  - 헤더 영역 스켈레톤
  - 테이블 영역 스켈레톤 (5줄)
  - 합계 영역 스켈레톤
- [ ] 실제 레이아웃과 동일한 구조
...
- [ ] `app/presentation/components/invoice/invoice-actions.tsx` 생성
...
- [ ] `app/presentation/routes/invoices/$invoiceId.tsx` 업데이트
  ...
  - 페이지 구조:
    ...
    5. Notes 섹션 (선택)
  - 더미 데이터로 렌더링
- [ ] meta 함수로 페이지 제목 설정 (Invoice #{invoice_number})
...
- [ ] `app/app.css`에 @media print 스타일 추가
...
- [ ] isLoading 상태일 때 InvoiceDetailSkeleton 표시
- [ ] 데이터가 있을 때 전체 인보이스 표시
- [ ] 에러 상태 처리 (Task 007과 연계)
- [ ] 조건부 렌더링 로직 구현
