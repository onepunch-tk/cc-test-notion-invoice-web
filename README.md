...
Ralph-Loop는 4단계 반복 디버깅 루프를 통해 웹 애플리케이션의 오류를 자동으로 수집, 분석, 해결, 테스트하는 통합 개발 스킬입니다.
...
```bash
/ralph-loop-playwright "해결하고자 하는 문제 또는 목표" [URL] [최대 시도 횟수] [인증 정보]
```
...
```bash
# 로그인 테스트
/ralph-loop-playwright "로그인 기능 테스트" http://localhost:5173/auth/signin 3 86tkstar@gmail.com xorudA2629!

# 페이지 404 에러 해결
/ralph-loop-playwright "개인정보 처리방침 페이지의 404에러를 해결해" http://localhost:5173

# UI 렌더링 오류 수정
/ralph-loop-playwright "header 컴포넌트가 제대로 렌더링되지 않음" http://localhost:5173
```
...
| 파라미터 | 필수 | 설명 |
|---------|------|------|
| goal | ✅ | 해결하고자 하는 문제 또는 달성 목표 |
| url | ❌ | 테스트 대상 URL (웹앱인 경우) |
| max | ❌ | 최대 루프 횟수 (미지정시 goal 달성까지 무한 반복) |
| email | ❌ | 로그인 테스트용 이메일 (인증 필요 시) |
| password | ❌ | 로그인 테스트용 패스워드 (인증 필요 시) |
